...
struct Loc {
    ...
    func statusInstruction(_ status: AgentStatus) -> String? {
        ...
        case (.muse, .missingAuth), (.muse, .notLoggedIn):
            switch lang {
            case .english: return "Run `muse login` in Terminal, then refresh."
            case .spanish: return "Ejecuta `muse login` en la Terminal, luego actualiza."
            case .french: return "Exécutez `muse login` dans le Terminal, puis actualisez."
            case .german: return "Führe `muse login` im Terminal aus und aktualisiere dann."
            case .japanese: return "ターミナルで `muse login` を実行し、更新してください。"
            case .korean: return "터미널에서 `muse login`을 실행한 후 새로고침하세요."
            case .chineseSimplified: return "在终端中运行 `muse login`，然后刷新。"
            }
        case (.muse, .accessDenied):
            switch lang {
            case .english: return "Allow Keychain access for the `ai.meta.dev.credentials` credential, then refresh."
            case .spanish: return "Permite el acceso al Llavero para las credenciales de `ai.meta.dev.credentials`, luego actualiza."
            case .french: return "Autorisez l'accès au Trousseau pour l'identifiant `ai.meta.dev.credentials`, puis actualisez."
            case .german: return "Erlaube den Schlüsselbund-Zugriff für Anmeldedaten von `ai.meta.dev.credentials`, danach aktualisieren."
            case .japanese: return "`ai.meta.dev.credentials` 資格情報のキーチェーンアクセスを許可し、更新してください。"
            case .korean: return "`ai.meta.dev.credentials` 자격 증명에 대한 키체인 접근을 허용한 후 새로고침하세요."
            case .chineseSimplified: return "允许对 `ai.meta.dev.credentials` 凭据的钥匙串访问，然后刷新。"
            }
        case (.muse, .sessionExpired):
            switch lang {
            case .english: return "Run `muse login` in Terminal and sign in again."
            case .spanish: return "Ejecuta `muse login` en la Terminal e inicia sesión de nuevo."
            case .french: return "Exécutez `muse login` dans le Terminal et connectez-vous à nouveau."
            case .german: return "Führe `muse login` im Terminal aus und melde dich erneut an."
